...
test("employee financial services cover request through payroll settlement", () => {
  const migration = readFileSync(
    "../../supabase/migrations/202608110017_financial_services_and_performance.sql",
    "utf8",
  );
  const loans = readFileSync(
    "src/app/[locale]/(protected)/loans/page.tsx",
    "utf8",
  );
  const statement = readFileSync(
    "src/app/[locale]/(protected)/loans/[loanId]/page.tsx",
    "utf8",
  );
...
test("performance management supports approved sales scoped targets and tiered bonuses", () => {
  ...
  const page = readFileSync(
    "src/app/[locale]/(protected)/performance/page.tsx",
    "utf8",
  );
  const planner = readFileSync(
    "src/components/performance-target-form.tsx",
    "utf8",
  );
  assert.match(migration, /create table public\.sales_entries/);
  assert.match(
    migration,
    /performance_scope as enum \('branch', 'team', 'employee'\)/,
  );
  assert.match(
    migration,
    /bonus_basis as enum \('fixed_amount', 'salary_percentage', 'sales_percentage'\)/,
  );
...
test("sales payroll and loans expose responsive decision-support charts", () => {
  ...
  const payroll = readFileSync(
    "src/app/[locale]/(protected)/payroll/page.tsx",
    "utf8",
  );
  const loans = readFileSync(
    "src/app/[locale]/(protected)/loans/page.tsx",
  ...
  assert.match(
    styles,
    /@media \(max-width: 1000px\)[\s\S]*\.section-insight-grid\s*\{[\s\S]*grid-template-columns:\s*1fr/,
  );
});
