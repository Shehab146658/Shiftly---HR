...
import {
  ...
  type AttendanceColumnOverrides,
  type AttendanceImportCell,
...
export async function updateBranchSchedulingRules(
  locale: AppLocale,
  tenantId: string,
  branchId: string,
  formData: FormData,
  ...
  const values = z
    ...
    .parse({
      ...
      isIndustrialEstablishment:
        formData.get("isIndustrialEstablishment") === "on",
      ...
      earlyDepartureGraceMinutes:
        formData.get("earlyDepartureGraceMinutes") || "0",
      overtimeThresholdMinutes:
        formData.get("overtimeThresholdMinutes") || "30",
      ...
      attendanceSelfieRequired:
        formData.get("attendanceSelfieRequired") === "on",
    });
  if (
    (values.geofenceLatitude === undefined) !==
    (values.geofenceLongitude === undefined)
  ) {
    throw new Error(
      "Enter both branch latitude and longitude, or leave both empty.",
    );
  }
  const supabase = await createSupabaseServerClient();
...
export async function setTeamMembers(
  ...
  const employeeIds = z
    .array(idSchema)
    .max(500)
    .parse([...new Set(formData.getAll("employeeIds").map(String))]);
  const supabase = await createSupabaseServerClient();
...
export async function assignAllEmployeesToTeam(
  locale: AppLocale,
  tenantId: string,
  teamId: string,
  _formData?: FormData,
...
const employeeFormSchema = z.object({
  ...
  branchId: optionalId,
  teamId: optionalId,
  managerEmployeeIds: z.array(idSchema).max(10),
...
function parseEmployeeForm(formData: FormData) {
  return employeeFormSchema.parse({
    ...
    managerEmployeeIds: [
      ...new Set(formData.getAll("managerEmployeeIds").map(String)),
    ],
...
function employeePayload(
  ...
  return {
    ...
    manager_employee_id: values.managerEmployeeIds[0] ?? null,
...
export async function createEmployee(
  ...
  const roleValue = optionalString(formData.get("roleId"));
  const roleId = roleValue ? idSchema.parse(roleValue) : null;
  const baseSalaryValue = optionalString(formData.get("baseSalary"));
  const compensation =
    baseSalaryValue === undefined
      ...
      : z
          ...
          .parse({
            salaryBasis: formData.get("salaryBasis") || "monthly",
            baseSalary: baseSalaryValue,
            currencyCode: formData.get("currencyCode") || "EGP",
            effectiveFrom:
              formData.get("salaryEffectiveFrom") ||
              values.hireDate ||
  ...
  const { error: managerError } = await supabase.rpc("set_employee_managers", {
    p_employee_id: employee.id,
    p_manager_ids: values.managerEmployeeIds,
  });
  ...
  if (compensation) {
    const usesBaseSalary = ["monthly", "mixed", "commission"].includes(
    ...
    const { error: compensationError } = await supabase.rpc(
      ...
      {
        p_employee_id: employee.id,
        p_salary_basis: compensation.salaryBasis,
        p_base_salary: usesBaseSalary ? compensation.baseSalary : 0,
        p_daily_rate:
          compensation.salaryBasis === "daily" ? compensation.baseSalary : null,
        p_hourly_rate:
          compensation.salaryBasis === "hourly"
            ? compensation.baseSalary
            : null,
        p_fixed_allowances: 0,
        p_currency_code: compensation.currencyCode,
        p_effective_from: compensation.effectiveFrom,
        p_notes: "Configured during employee creation",
      },
    );
...
export async function updateRolePermissions(
  ...
  const permissionKeys = z
    ...
    .max(100)
    .parse([...new Set(formData.getAll("permissionKeys").map(String))]);
  const supabase = await createSupabaseServerClient();
...
export async function updateEmployee(
  ...
  if (error) throw error;
  const { error: managerError } = await supabase.rpc("set_employee_managers", {
    p_employee_id: idSchema.parse(employeeId),
    p_manager_ids: values.managerEmployeeIds,
  });
  if (managerError) throw managerError;
  revalidatePath(`/${locale}/employees`);
...
export async function submitHrRequest(
  ...
  if (hasDocument) {
    ...
    if (uploadError) {
      await supabase.rpc("cancel_hr_request", {
        p_request_id: parsedRequestId,
        p_reason: "Supporting document upload failed",
      });
      throw uploadError;
    }
    ...
    if (attachError) {
      ...
      await supabase.rpc("cancel_hr_request", {
        p_request_id: parsedRequestId,
        p_reason: "Supporting document attachment failed",
      });
      throw attachError;
    }
  }
...
export async function reviewHrRequest(
  ...
  const note = z
    ...
    .max(2000)
    .optional()
    .parse(optionalString(formData.get("reviewNote")));
  const supabase = await createSupabaseServerClient();
...
export async function cancelHrRequest(
  ...
  const reason = z
    ...
    .min(2)
    .max(1000)
    .parse(formData.get("cancellationReason"));
  const supabase = await createSupabaseServerClient();
...
export async function cloneRequestWorkflow(
  locale: AppLocale,
  workflowId: string,
  formData: FormData,
) {
...
export async function addRequestWorkflowStep(
  locale: AppLocale,
  tenantId: string,
  workflowId: string,
  formData: FormData,
  ...
  if (values.approverKind === "role" && !values.roleId)
    throw new Error("Choose a role for a role-based approval step.");
  const supabase = await createSupabaseServerClient();
...
export async function updateRequestWorkflowStep(
  locale: AppLocale,
  tenantId: string,
  stepId: string,
  formData: FormData,
  ...
  const { error } = await supabase
    ...
    .eq("tenant_id", idSchema.parse(tenantId))
    .eq("id", idSchema.parse(stepId));
  if (error) throw error;
  refreshRequestPaths(locale);
}

export async function deleteRequestWorkflowStep(
  locale: AppLocale,
  tenantId: string,
  stepId: string,
) {
...
export async function activateRequestWorkflow(
  locale: AppLocale,
  workflowId: string,
) {
  const supabase = await createSupabaseServerClient();
  const { error } = await supabase.rpc("activate_request_workflow", {
    p_workflow_id: idSchema.parse(workflowId),
  });
  if (error) throw error;
  refreshRequestPaths(locale);
...
export async function markNotificationRead(
  ...
  const { error } = await supabase.rpc("mark_notification_read", {
    p_notification_id: idSchema.parse(notificationId),
  });
  if (error) throw error;
...
const leaveDayPartSchema = z.enum([
  "full",
  "first_half",
  "second_half",
  "hours",
]);
const leaveTransactionKindSchema = z.enum([
  "adjustment",
  "carryover",
  "settlement",
  "holiday_credit",
  ...
  if (hasDocument) {
    ...
    if (uploadError) {
      await supabase.rpc("cancel_leave_request", {
        p_request_id: parsedRequestId,
        p_reason: "Document upload failed",
      });
      throw uploadError;
    }
    ...
    if (attachError) {
      await supabase.rpc("cancel_leave_request", {
        p_request_id: parsedRequestId,
        p_reason: "Document attachment failed",
      });
      throw attachError;
    }
  }
...
export async function deletePublicHoliday(
  ...
  const { error } = await supabase
    ...
    .delete()
    .eq("tenant_id", idSchema.parse(tenantId))
    .eq("id", idSchema.parse(holidayId));
  if (error) throw error;
  refreshLeavePaths(locale);
}
...
export async function updateLeaveType(
  ...
  const { error } = await supabase
    ...
    .eq("tenant_id", idSchema.parse(tenantId))
    .eq("id", idSchema.parse(leaveTypeId));
  if (error) throw error;
  refreshLeavePaths(locale);
}
...
const payrollStatusSchema = z.enum([
  "reviewed",
  "approved",
  "locked",
  "published",
...
export async function updatePayrollSettings(
  ...
  const values = z
    ...
    .parse({
      ...
      lateDeductionMultiplier: formData.get("lateDeductionMultiplier"),
      absenceDeductionMultiplier: formData.get("absenceDeductionMultiplier"),
      roundToDigits: formData.get("roundToDigits"),
  ...
  const { error } = await supabase
    ...
    .update({
      ...
      standard_monthly_days: values.standardMonthlyDays,
      standard_daily_hours: values.standardDailyHours,
      overtime_multiplier: values.overtimeMultiplier,
      late_deduction_multiplier: values.lateDeductionMultiplier,
      absence_deduction_multiplier: values.absenceDeductionMultiplier,
      round_to_digits: values.roundToDigits,
      tax_enabled: values.taxEnabled,
      insurance_enabled: values.insuranceEnabled,
    })
    .eq("tenant_id", idSchema.parse(tenantId));
  if (error) throw error;
...
export async function saveEmployeeCompensation(
  locale: AppLocale,
  employeeId: string,
  formData: FormData,
) {
  const values = z
    ...
    .parse({
      ...
      dailyRate: optionalString(formData.get("dailyRate")),
      hourlyRate: optionalString(formData.get("hourlyRate")),
      fixedAllowances: formData.get("fixedAllowances") || "0",
      currencyCode: formData.get("currencyCode"),
  ...
  const { error } = await supabase.rpc("upsert_employee_compensation", {
    p_employee_id: idSchema.parse(employeeId),
    p_salary_basis: values.salaryBasis,
    p_base_salary: values.baseSalary,
    p_daily_rate: values.dailyRate ?? null,
    p_hourly_rate: values.hourlyRate ?? null,
    p_fixed_allowances: values.fixedAllowances,
...
export async function createPayrollPeriod(
  ...
  const values = z
    .object({
      ...
      startDate: dateSchema,
      endDate: dateSchema,
      payDate: dateSchema.optional(),
    })
    .parse({
      name: formData.get("name"),
      startDate: formData.get("startDate"),
      endDate: formData.get("endDate"),
      payDate: optionalString(formData.get("payDate")),
    });
  const supabase = await createSupabaseServerClient();
  const { data, error } = await supabase.rpc("create_payroll_period", {
    ...
    p_code: null,
    p_name: values.name,
    p_start: values.startDate,
    p_end: values.endDate,
    p_pay_date: values.payDate ?? null,
  });
  if (error) throw error;
...
export async function calculatePayrollPeriod(
  ...
  const { error } = await supabase.rpc("calculate_payroll_period", {
    p_period_id: idSchema.parse(periodId),
  });
  if (error) throw error;
...
export async function transitionPayrollPeriod(
  locale: AppLocale,
  periodId: string,
  target: "reviewed" | "approved" | "locked" | "published" | "cancelled",
  formData: FormData,
) {
  const note = z
    .string()
...
export async function addPayrollAdjustment(
  ...
  const adjustmentCode = `manual_${values.kind}_${randomUUID().replaceAll("-", "").slice(0, 10)}`;
  const supabase = await createSupabaseServerClient();
  const { error } = await supabase.rpc("add_payroll_adjustment", {
    p_result_id: idSchema.parse(resultId),
    p_kind: values.kind,
    p_code: adjustmentCode,
    p_name_en: values.nameEn,
    p_name_ar: values.nameAr,
...
export async function deletePayrollAdjustment(
  ...
  const { error } = await supabase.rpc("delete_payroll_adjustment", {
    p_component_id: idSchema.parse(componentId),
  });
  if (error) throw error;
  refreshPayrollPaths(locale, periodId, resultId);
}

export async function acknowledgePayslip(
  locale: AppLocale,
  resultId: string,
  payslipId: string,
) {
  const supabase = await createSupabaseServerClient();
  const { error } = await supabase.rpc("acknowledge_payslip", {
    p_payslip_id: idSchema.parse(payslipId),
  });
  if (error) throw error;
  refreshPayrollPaths(locale, undefined, resultId);
}

const loanPaymentMethodSchema = z.enum([
  "cash",
  "bank_transfer",
  "settlement",
  "adjustment",
...
export async function submitLoanRequest(
  ...
  const values = z
    ...
    .parse({
      employeeId:
        optionalString(formData.get("employeeId")) ?? defaultEmployeeId,
      amount: formData.get("amount"),
  ...
  const { error } = await supabase.rpc("submit_loan_request", {
    ...
    p_start_month: values.startMonth,
...
export async function cancelLoanRequest(
  ...
  const reason = z
    ...
    .max(1000)
    .optional()
    .parse(optionalString(formData.get("reason")));
  const supabase = await createSupabaseServerClient();
...
export async function reviewLoanRequest(
  ...
  const values = z
    .object({
      ...
      startMonth: dateSchema.optional(),
      note: z.string().trim().max(2000).optional(),
    })
    .parse({
      amount: optionalString(formData.get("approvedAmount")),
      installments: optionalString(formData.get("approvedInstallments")),
      startMonth: optionalString(formData.get("approvedStartMonth")),
      note: optionalString(formData.get("decisionNote")),
    });
  if (approve && (!values.amount || !values.installments || !values.startMonth))
    throw new Error(
      "Approved amount, installment count, and start month are required.",
    );
  if (!approve && (!values.note || values.note.length < 2))
  ...
  const { data, error } = await supabase.rpc("review_loan_request", {
    ...
    p_installments: values.installments ?? 1,
    p_start_month:
      values.startMonth ?? new Date().toISOString().slice(0, 8) + "01",
    p_note: values.note ?? "",
  });
  if (error) throw error;
...
export async function recordLoanPayment(
  ...
  const values = z
    .object({
      ...
      method: loanPaymentMethodSchema,
  ...
  const { error } = await supabase.rpc("record_loan_payment", {
    p_loan_id: idSchema.parse(loanId),
    p_amount: values.amount,
...
export async function rescheduleLoanInstallment(
  locale: AppLocale,
  loanId: string,
  ...
  const values = z
    .object({ dueDate: dateSchema, reason: z.string().trim().min(2).max(1000) })
    .parse({
      dueDate: formData.get("dueDate"),
      reason: formData.get("reason"),
    });
  const supabase = await createSupabaseServerClient();
  const { error } = await supabase.rpc("reschedule_loan_installment", {
    p_installment_id: idSchema.parse(installmentId),
...
export async function setEmployeeLoanStatus(
  locale: AppLocale,
  loanId: string,
  nextStatus: "active" | "paused" | "written_off",
  formData: FormData,
) {
  ...
  const note = z
    ...
    .max(2000)
    .optional()
    .parse(optionalString(formData.get("statusNote")));
  if (status === "written_off" && (!note || note.length < 2))
    throw new Error("A write-off reason is required.");
  const supabase = await createSupabaseServerClient();
  const { error } = await supabase.rpc("set_employee_loan_status", {
    p_loan_id: idSchema.parse(loanId),
    p_status: status,
    p_note: note ?? "",
  });
...
export async function recordSalesEntry(
  locale: AppLocale,
  tenantId: string,
  formData: FormData,
) {
...
export async function reviewSalesEntry(
  locale: AppLocale,
  entryId: string,
  approve: boolean,
  formData: FormData,
  ...
  const note = z
    ...
    .optional()
    .parse(optionalString(formData.get("reviewNote")));
  if (!approve && (!note || note.length < 2))
    throw new Error("A rejection reason is required.");
  const supabase = await createSupabaseServerClient();
  const { error } = await supabase.rpc("review_sales_entry", {
    p_entry_id: idSchema.parse(entryId),
    p_approve: approve,
    p_note: note ?? "",
  });
  if (error) throw error;
...
export async function createBonusPolicy(
  ...
  const values = z
    .object({
      nameEn: z.string().trim().min(2).max(150),
      nameAr: z.string().trim().max(150).optional(),
      basis: bonusBasisSchema,
      thresholdOne: z.coerce.number().min(0).max(10000),
      valueOne: z.coerce.number().min(0).max(1_000_000),
      thresholdTwo: z.coerce.number().min(0).max(10000),
      valueTwo: z.coerce.number().min(0).max(1_000_000),
      thresholdThree: z.coerce.number().min(0).max(10000).optional(),
      valueThree: z.coerce.number().min(0).max(1_000_000).optional(),
      effectiveFrom: dateSchema,
    ...
    .parse({
      ...
      thresholdOne: formData.get("thresholdOne"),
      valueOne: formData.get("valueOne"),
      thresholdTwo: formData.get("thresholdTwo"),
      valueTwo: formData.get("valueTwo"),
      thresholdThree: optionalString(formData.get("thresholdThree")),
      valueThree: optionalString(formData.get("valueThree")),
      effectiveFrom: formData.get("effectiveFrom"),
      effectiveTo: optionalString(formData.get("effectiveTo")),
    });
  const tiers = [
    { min_percentage: values.thresholdOne, value: values.valueOne },
    { min_percentage: values.thresholdTwo, value: values.valueTwo },
  ];
  if (values.thresholdThree !== undefined && values.valueThree !== undefined)
    tiers.push({
      min_percentage: values.thresholdThree,
      value: values.valueThree,
    });
  const supabase = await createSupabaseServerClient();
  const { error } = await supabase.rpc("create_bonus_policy", {
    p_tenant_id: idSchema.parse(tenantId),
    p_code: null,
    p_name_en: values.nameEn,
    p_name_ar: values.nameAr ?? "",
    ...
    p_tiers: tiers,
    p_effective_from: values.effectiveFrom,
    p_effective_to: values.effectiveTo ?? null,
  });
  if (error) throw error;
  refreshPerformancePaths(locale);
}
...
export async function createSalesTarget(
  ...
  const values = z
    ...
    .refine((value) => value.endDate >= value.startDate, {
      message: "Target end must be on or after the start.",
    })
    .parse({
      name: formData.get("name"),
      startDate: formData.get("startDate"),
  ...
  const supabase = await createSupabaseServerClient();
  const { error } = await supabase.rpc("create_sales_target", {
    p_tenant_id: idSchema.parse(tenantId),
    p_code: null,
    p_name: values.name,
    p_start: values.startDate,
...
export async function calculateBonusTarget(
  locale: AppLocale,
  targetId: string,
) {
  const supabase = await createSupabaseServerClient();
  const { error } = await supabase.rpc("calculate_bonus_target", {
    p_target_id: idSchema.parse(targetId),
  });
  if (error) throw error;
  refreshPerformancePaths(locale);
}

export async function reviewBonusTarget(
  locale: AppLocale,
  targetId: string,
  approve: boolean,
  formData: FormData,
  ...
  if (!approve && (!note || note.length < 2))
    throw new Error("A rejection reason is required.");
  const supabase = await createSupabaseServerClient();
  const { error } = await supabase.rpc("review_bonus_target", {
    p_target_id: idSchema.parse(targetId),
    p_approve: approve,
    p_note: note ?? "",
  });
...
const announcementScopeSchema = z.enum([
  "company",
  "branches",
  "teams",
  "employees",
...
export async function createOperationalTask(
  ...
  const values = z
    .object({
      ...
      descriptionEn: z.string().trim().min(2).max(5000),
    ...
    .refine((value) => new Date(value.dueAt) > new Date(value.startAt), {
      message: "The due time must follow the start time.",
    })
    .refine(
      (value) => value.recurrence !== "none" || !value.recurrenceEndDate,
      { message: "One-time tasks cannot have a recurrence end date." },
    )
    .parse({
      titleEn: formData.get("titleEn"),
      titleAr: optionalString(formData.get("titleAr")),
      descriptionEn: formData.get("descriptionEn"),
      descriptionAr: optionalString(formData.get("descriptionAr")),
      ...
      recurrenceEndDate: optionalString(formData.get("recurrenceEndDate")),
    });
  const scopeIds = formData
    .getAll("scopeIds")
    .map(String)
    .filter(Boolean)
    .map((value) => idSchema.parse(value));
  if (scope !== "company" && !scopeIds.length)
    throw new Error("Select at least one employee, team, or branch.");
  const supabase = await createSupabaseServerClient();
  const { data, error } = await supabase.rpc("create_operational_task", {
    p_tenant_id: idSchema.parse(tenantId),
    p_title_en: values.titleEn,
    p_title_ar: values.titleAr ?? "",
    p_description_en: values.descriptionEn,
    p_description_ar: values.descriptionAr ?? "",
    p_priority: values.priority,
    p_start_at: new Date(values.startAt).toISOString(),
    p_due_at: new Date(values.dueAt).toISOString(),
    p_require_evidence: values.requireEvidence,
    p_recurrence: values.recurrence,
    p_recurrence_interval: values.recurrenceInterval,
    p_recurrence_end_date: values.recurrenceEndDate ?? null,
    p_scope: scope,
...
export async function startTaskAssignment(
  ...
  const supabase = await createSupabaseServerClient();
  const { error } = await supabase.rpc("start_task_assignment", {
    p_assignment_id: idSchema.parse(assignmentId),
  });
  if (error) throw error;
...
export async function submitTaskEvidence(
  ...
  const notes = z
    ...
    .max(2000)
    .optional()
    .parse(optionalString(formData.get("notes")));
  ...
  try {
    ...
    if (paths.length)
      await supabase.storage.from("task-evidence").remove(paths);
    throw error;
  }
  refreshTaskPaths(locale, taskId);
}
...
export async function reviewTaskEvidence(
  ...
  const supabase = await createSupabaseServerClient();
  const { error } = await supabase.rpc("review_task_assignment", {
    p_assignment_id: idSchema.parse(assignmentId),
    p_approve: approve,
    p_note: note ?? "",
  });
...
export async function addTaskComment(
  ...
  const { error } = await supabase.rpc("add_task_comment", {
    p_task_id: idSchema.parse(taskId),
    p_body: body,
  });
  if (error) throw error;
  refreshTaskPaths(locale, taskId);
...
export async function cancelOperationalTask(
  ...
  const supabase = await createSupabaseServerClient();
  const { error } = await supabase.rpc("cancel_operational_task", {
    p_task_id: idSchema.parse(taskId),
    p_reason: reason,
  });
  if (error) throw error;
...
export async function createAnnouncement(
  ...
  const values = z
    .object({
      titleEn: z.string().trim().min(2).max(180),
      titleAr: z.string().trim().max(180).optional(),
      bodyEn: z.string().trim().min(2).max(10000),
      bodyAr: z.string().trim().max(10000).optional(),
    ...
    .parse({
      ...
      bodyAr: optionalString(formData.get("bodyAr")),
      priority: formData.get("priority"),
  ...
  const { data, error } = await supabase.rpc("create_announcement", {
    ...
    p_title_en: values.titleEn,
    p_title_ar: values.titleAr ?? "",
    p_body_en: values.bodyEn,
    p_body_ar: values.bodyAr ?? "",
    p_priority: values.priority,
    p_is_pinned: values.pinned,
    p_requires_acknowledgement: values.acknowledgement,
...
export async function publishAnnouncement(
  ...
  const { error } = await supabase.rpc("publish_announcement", {
    p_announcement_id: idSchema.parse(announcementId),
  });
  if (error) throw error;
  refreshAnnouncementPaths(locale);
}
...
export async function toggleShiftTemplate(
  locale: AppLocale,
  tenantId: string,
  shiftId: string,
  nextActive: boolean,
...
export async function createWeeklySchedule(
  ...
  const normalizedWeekStart = configuredWeekStart(
    values.weekStart,
    branch.week_start_isodow,
  );
...
export async function addScheduleEntry(
  ...
  const values = z
    .object({
      ...
      workDate: dateSchema,
      assignmentMode: z.enum(["replace", "append"]),
      ...
      if (value.entryType === "shift") {
        ...
        if (usesTemplate === usesCustom) {
          ctx.addIssue({
            code: "custom",
            message:
              "Choose either a shift template or custom start/end times.",
          });
        }
      }
    })
    .parse({
      ...
      assignmentMode: formData.get("assignmentMode") || "replace",
  ...
  const supabase = await createSupabaseServerClient();
  let segmentNo = 1;
  if (isShift && values.assignmentMode === "append") {
    const { data: existing, error: existingError } = await supabase
      ...
      .order("segment_no", { ascending: false })
      .limit(1);
    if (existingError) throw existingError;
    const currentSegment = Number(existing?.[0]?.segment_no ?? 0);
    if (currentSegment >= 10) {
      throw new Error("A workday can contain up to 10 split-shift segments.");
    }
    segmentNo = currentSegment + 1;
  } else {
...
export async function bulkAssignScheduleEntries(
  locale: AppLocale,
  tenantId: string,
  scheduleId: string,
  formData: FormData,
    ...
    .superRefine((value, ctx) => {
      ...
      if (usesTemplate === usesCustom) {
        ctx.addIssue({
          code: "custom",
          message:
            "Choose a shift template or enter exact start and end times.",
        });
      }
    })
    .parse({
  ...
  const usesCustomTimes =
    values.entryType === "shift" && !values.shiftTemplateId;
  const endsNextDay =
    usesCustomTimes &&
  ...
  const { error } = await supabase.rpc("bulk_assign_schedule_entries", {
    ...
    p_shift_template_id:
      values.entryType === "shift" ? (values.shiftTemplateId ?? null) : null,
    p_custom_start_time: usesCustomTimes
      ? (values.customStartTime ?? null)
      : null,
...
export async function transitionSchedule(
  locale: AppLocale,
  scheduleId: string,
  targetStatus: "draft" | "published" | "locked" | "archived",
  formData?: FormData,
) {
...
export async function recordManualAttendancePunch(
  locale: AppLocale,
  tenantId: string,
  formData: FormData,
) {
...
export async function refreshAttendancePeriod(
  locale: AppLocale,
  tenantId: string,
  formData: FormData,
) {
...
export async function reviewAttendancePunch(
  ...
  const { error } = await supabase.rpc("review_attendance_punch", {
    p_punch_id: idSchema.parse(punchId),
    p_decision: decision,
    p_note: note ?? null,
  });
  if (error) throw error;
...
export async function importFingerprintAttendance(
  locale: AppLocale,
  tenantId: string,
  formData: FormData,
) {
  ...
  if (!(fileValue instanceof File) || !fileValue.name || fileValue.size < 1)
    throw new Error("Choose a CSV or XLSX attendance file.");
  if (fileValue.size > 8 * 1024 * 1024)
    throw new Error("Attendance files are limited to 8 MB.");
  const extension = fileValue.name.split(".").pop()?.toLowerCase();
  if (!extension || !["csv", "txt", "xlsx"].includes(extension))
    throw new Error("Use a CSV, TXT, or XLSX attendance file.");
  ...
  const parsed =
    extension === "xlsx"
      ? parseAttendanceTable(
          ...
          ).readSheet(fileBuffer)) as AttendanceImportCell[][],
          overrides,
        )
