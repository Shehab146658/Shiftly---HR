  ...
  return <>
    ...
    <div className="workflow-type-list">{(typeData ?? []).map((type) => {
      ...
      return <section className="card workflow-type-card" key={type.id}>
        ...
        {canManageWorkflows && active ? <details className="workflow-clone"><summary>{copy.clone}</summary><ActionForm action={cloneRequestWorkflow.bind(null, locale, active.id)} className="form-grid compact-form" errorMessage={copy.actionFailed} pendingMessage={copy.saving} successMessage={copy.draftCreated}><div className="field"><label>{copy.nameEn}</label><input className="input" defaultValue={`${active.name_en} v${active.version + 1}`} name="nameEn" required /></div><div className="field"><label>{copy.nameAr}</label><input className="input" defaultValue={`${active.name_ar} ${active.version + 1}`} name="nameAr" required /></div><button className="button full" type="submit">{copy.createDraft}</button></ActionForm></details> : null}
