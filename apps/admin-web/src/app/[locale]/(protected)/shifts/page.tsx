...
export default async function ShiftTemplatesPage({
  ...
  const [{ data: shifts, error }, { data: branches }] = await Promise.all([
    supabase
      ...
      .select(
        "id, code, name_en, name_ar, start_time, end_time, end_day_offset, break_minutes, color_hex, is_active, branches(name_en)",
      )
  ...
  return (
    <>
      <div className="page-head">
        <div>
          <h1 className="page-title">{d.shifts}</h1>
          <p className="muted">
            {shifts?.length ?? 0} {d.shifts.toLowerCase()}
          </p>
        </div>
        {canManage ? (
          <CreateDialog
            ...
            description={
              locale === "ar"
                ? "أنشئ وردية قابلة لإعادة الاستخدام، بما في ذلك الورديات الليلية وفترات الراحة."
                : "Create a reusable working pattern, including overnight shifts and breaks."
            }
            eyebrow={d.shifts}
            title={addShift}
            triggerLabel={addShift}
          >
            <ActionForm
              action={action}
            ...
            >
              <div className="automatic-record-note full">
                ...
                <div>
                  <strong>
                    {locale === "ar"
                      ? "كود الوردية تلقائي"
                      : "Automatic shift code"}
                  </strong>
                  <small>
                    {locale === "ar"
                      ? "يُنشأ عند حفظ قالب الوردية."
                      : "Generated when this reusable shift is saved."}
                  </small>
                </div>
              </div>
              <div className="field">
              ...
              <div className="field">
                ...
                <select className="select" name="branchId">
                  <option value="">{d.companyWide}</option>
                  {branches?.map((b) => (
              ...
              <div className="field">
                <label>{d.endTime}</label>
                <input className="input" type="time" name="endTime" required />
              </div>
              <div className="field">
                <label>{d.nextDay}</label>
                <select className="select" name="endDayOffset">
                  <option value="0">No</option>
                  <option value="1">Yes</option>
                </select>
              </div>
              <div className="field">
                <label>{d.breakMinutes}</label>
                <input
                  className="input"
                  type="number"
                  min="0"
              ...
              <div className="full">
                <button className="button">{addShift}</button>
              </div>
            </ActionForm>
          </CreateDialog>
        ) : null}
      ...
      <section className="card stack section-gap">
        <div className="table-wrap">
            ...
            <tbody>
              {shifts?.map((row) => {
                const branch = Array.isArray(row.branches)
                  ? row.branches[0]
                  : row.branches;
                const toggleAction = toggleShiftTemplate.bind(
                  null,
                  locale,
                  tenantId,
                  row.id,
                ...
                return (
                  <tr key={row.id}>
                    <td>
                      <span
                        className="color-dot"
                        style={{ background: row.color_hex ?? "#2357D9" }}
                      />
                      ...
                      {canManage ? (
                        ...
                        >
                          <button className="text-button" type="submit">
                            {row.is_active ? d.deactivate : d.activate}
                          </button>
                        </ActionForm>
                      ) : (
                        "—"
          ...
          {!shifts?.length ? <div className="empty">{d.empty}</div> : null}
        </div>
      </section>
    </>
  );
