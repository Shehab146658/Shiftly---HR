...
export default async function TasksPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return <>
    <div className="page-head"><div><h1 className="page-title">{copy.title}</h1><p className="muted">{copy.subtitle}</p></div>{canCreate ? <CreateDialog closeLabel={d.close} description={copy.subtitle} eyebrow={copy.allWork} title={copy.newTask} triggerLabel={copy.newTask}><TaskComposer action={createOperationalTask.bind(null, locale, tenantId)} branches={(branches ?? []).map((branch) => ({ id: branch.id, label: locale === "ar" && branch.name_ar ? branch.name_ar : branch.name_en, meta: branch.code }))} defaults={{ startAt: localInput(start), dueAt: localInput(due) }} employees={(employees ?? []).map((employee) => ({ id: employee.id, label: locale === "ar" && employee.name_ar ? employee.name_ar : employee.name_en, meta: `${employee.employee_code} · ${employee.position ?? d.noPosition}` }))} locale={locale} teams={(teams ?? []).map((team) => ({ id: team.id, label: locale === "ar" && team.name_ar ? team.name_ar : team.name_en, meta: team.code }))} /></CreateDialog> : null}</div>
