...
export default async function LeaveSettingsPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return <>
    ...
    <div className="settings-two-column section-gap">
      <section className="card stack"><div><h2>{copy.balances}</h2><p className="muted">{copy.balancesHelp}</p></div><ActionForm action={adjustLeaveBalance.bind(null, locale)} className="form-grid" errorMessage={copy.actionFailed} pendingMessage={copy.saving} successMessage={copy.adjusted} resetOnSuccess>
      ...
      <section className="card stack"><div><h2>{copy.holidays}</h2><p className="muted">{copy.holidaysHelp}</p></div><ActionForm action={createPublicHoliday.bind(null, locale, tenantId)} className="form-grid" errorMessage={copy.actionFailed} pendingMessage={copy.saving} successMessage={copy.holidayAdded} resetOnSuccess>
